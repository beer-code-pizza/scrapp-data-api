...
for category, firstUrl in categoryUrls:
    try:
        print("Category: ", category)
        firstUrl = f"https://www.prezunic.com.br/{category}?page="
        dataTransformation(getItensAndPrices(getWithBs4(firstUrl+str(1))),source).to_csv(f"{path}/itens_precos_{category}_{source}.csv", header=True, sep = ';', index = False)
