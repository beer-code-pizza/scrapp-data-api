...
from utils.modules.scrapper import getWithBs4
from utils.modules.mercadolivre import getItensAndPrices, getNextUrl
...
for category, firstUrl in firstUrls.items():
    ...
    getItensAndPrices(getWithBs4(firstUrl)).to_csv(f"{path}/itens_precos_{category}_mercadolivre.csv", sep = ';', index = False)
    try:
    ...
    try:
        while next_url:
            ...
            getItensAndPrices(getWithBs4(next_url)).to_csv(f"{path}/itens_precos_{category}_mercadolivre.csv", sep = ';',header = False, index = False, mode='a')
